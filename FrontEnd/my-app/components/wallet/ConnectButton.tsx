...
export function ConnectButton() {
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="group pointer-events-auto flex cursor-pointer items-center gap-2 rounded-s-2xl border border-zinc-300 bg-white px-3 py-2 transition-all hover:border-secondary/50 dark:border-dark-border-darker dark:bg-dark-surface sm:gap-3 sm:px-6 sm:py-3"
        >
          <div className="h-2 w-2 rounded-full bg-secondary shadow-glow-sm" />
          <span className="font-medium tracking-wide text-secondary">
        ...
        <AnimatePresence>
          {dropdownOpen && (
            <motion.div
              ...
              className="absolute right-0 top-full z-50 mt-2 w-full min-w-[180px] overflow-hidden rounded-xl border border-zinc-300 bg-white shadow-xl dark:border-dark-border-darker dark:bg-dark-surface"
            >
              <button
                type="button"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 rounded-s-2xl border border-zinc-300 bg-white px-4 py-2 text-sm font-medium tracking-wide text-secondary shadow-lg shadow-black/10 transition-all hover:border-secondary/50 dark:border-dark-border-darker dark:bg-dark-surface dark:shadow-black/20 sm:gap-4 sm:px-8 sm:py-3 sm:text-base">
        <span>Connect Wallet</span>
        <ArrowDownIcon />
      </div>
      <div className="hidden h-8 w-1.5 items-center justify-center bg-zinc-300 transition-colors dark:bg-dark-surface-elevated sm:flex" />
    </motion.button>
  );
}
